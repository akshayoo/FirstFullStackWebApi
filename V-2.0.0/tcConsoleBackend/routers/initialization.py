...
async def form_fetch_mail(payload: ProjectSubmission, usertok : dict = Depends(parse_token)):
    ...
    for stakeholder in payload.stakeholders:
        s = stakeholder if isinstance(stakeholder, dict) else stakeholder.model_dump()     
        if s.get("name", "").strip() and s.get("email", "").strip():
            project_stakeholders.append(s)
    
    emails = [s.get("email") for s in project_stakeholders]
    ...
    try:
        ...
        html_cont = template.render(
            ...
            submission_link = f"https://console.theracues.com/sample-submissions?token={project_token}",

            standard_deliverables = payload.standard_deliverables,
            added_deliverables = payload.added_deliverables,
        ) 

        cc_emails = [usertok["username"], "cues@theracues.com", "analysis@theracues.com", "projectmgt@theracues.com"]
